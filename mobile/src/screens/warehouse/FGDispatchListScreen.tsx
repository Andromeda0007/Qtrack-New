...
const EmptyState = () => (
  <View style={styles.empty}>
    ...
    <Text style={styles.emptyTitle}>No FG in warehouse stock</Text>
    <Text style={styles.emptySub}>Received FG batches available for dispatch will appear here.</Text>
  </View>
);
...
const FGCard: React.FC<{ item: FGBatchListItem; onPress: () => void }> = ({ item, onPress }) => (
  <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.85}>
    ...
    <View style={styles.dispatchRow}>
      ...
      <Text style={styles.dispatchText}>Tap to dispatch to customer</Text>
    </View>
  </TouchableOpacity>
);

export const FGDispatchListScreen: React.FC = () => {
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <FGCard
              item={item}
              onPress={() => navigation.navigate('DispatchFG', { batch: item })}
            />
          )}
        />
      )}
...
const styles = StyleSheet.create({
  ...
  stockBadgeText: { fontSize: FontSize.xs, fontWeight: '700', color: Colors.primary },
  ...
  dispatchText: { fontSize: FontSize.xs, color: Colors.primary, fontWeight: '700' },
