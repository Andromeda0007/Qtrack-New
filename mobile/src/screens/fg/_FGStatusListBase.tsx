...
export const FGStatusListBase: React.FC<Props> = ({
  status, title, bgColor, textColor, icon, emptyMessage,
}) => {
  ...
  const load = useCallback(async () => {
    try {
      const data = await productionApi.listFGBatches(status);
      setBatches(data);
    } catch {
    } finally {
      setLoading(false);
  ...
  const renderItem = ({ item }: { item: any }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('FGBatchDetail', { fgBatchId: item.id, batch: item })}
      activeOpacity={0.8}
    >
      <View style={styles.cardInner}>
        <View style={styles.cardContent}>
          <View style={[styles.idBadge, { backgroundColor: bgColor }]}>
            <Text style={[styles.idText, { color: textColor }]}>#{item.id}</Text>
          </View>
          <Text style={styles.itemCode}>{item.batch_number || '—'}</Text>
          <Text style={styles.itemName}>{item.product_name || '—'}</Text>
          <View style={styles.cardMeta}>
            <View style={styles.metaItem}>
              <Ionicons name="cube-outline" size={13} color={Colors.textMuted} />
              <Text style={styles.metaText} numberOfLines={1}>{item.quantity ?? '—'} units</Text>
            </View>
        ...
        <Ionicons name="chevron-forward" size={18} color={textColor} style={styles.chevron} />
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <View style={styles.searchRow}>
        <SearchInput value={search} onChangeText={setSearch} placeholder="Search batch or product" />
      </View>
      ...
      <View style={styles.sortRow}>
        {SORT_OPTIONS.map((opt) => (
          <TouchableOpacity
            key={opt.value}
            style={[styles.sortChip, sort === opt.value && { backgroundColor: bgColor, borderColor: textColor }]}
            onPress={() => setSort(opt.value)}
          >
            <Ionicons name={opt.icon as any} size={12} color={sort === opt.value ? textColor : Colors.textSecondary} />
            <Text style={[styles.sortLabel, sort === opt.value && { color: textColor }]}>{opt.label}</Text>
          </TouchableOpacity>
        ))}
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyText}>{emptyMessage ?? `No ${title.toLowerCase()} batches`}</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  sortRow: { flexDirection: 'row', gap: Spacing.xs, paddingHorizontal: Spacing.md, paddingBottom: Spacing.sm },
  sortChip: {
  ...
  countLabel: { fontSize: FontSize.xs, color: Colors.textMuted, fontWeight: '600', marginBottom: Spacing.sm, marginTop: Spacing.sm },
  card: {
  ...
  cardContent: { flex: 1 },
  idBadge: { borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3, alignSelf: 'flex-start', marginBottom: 8 },
