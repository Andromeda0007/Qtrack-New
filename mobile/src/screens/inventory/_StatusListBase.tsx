...
export const StatusListBase: React.FC<Props> = ({ status, title, bgColor, textColor, icon }) => {
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <View style={styles.sortRow}>
        {SORT_OPTIONS.map((opt) => (
          <TouchableOpacity
            ...
            style={[
              styles.sortChip,
              sort === opt.value && { backgroundColor: bgColor, borderColor: textColor },
            ]}
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <Text style={styles.countLabel}>{displayed.length} {displayed.length === 1 ? 'product' : 'products'}</Text>
          }
          ListEmptyComponent={
            <View style={styles.empty}>
              <Ionicons name="cube-outline" size={48} color={Colors.textMuted} />
              <Text style={styles.emptyText}>No {title.toLowerCase()} cards</Text>
            </View>
          }
        />
      )}
